...
export default function PromptStudio({ className }: PromptStudioProps) {
  const [activeTab, setActiveTab] = useState<"current" | "edit" | "test">(
    "current"
  );
  const [editedAgentPrompt, setEditedAgentPrompt] = useState(
    DEFAULT_PROMPTS.agent
  );
  const [editedRegularPrompt, setEditedRegularPrompt] = useState(
  ...
  const testPrompt = async () => {
    ...
    try {
      ...
      const reasoningProvider =
        localStorage.getItem("reasoningProvider") || "openai";
      ...
      if (!reasoningModel) {
        setTestResult(
          "No reasoning model selected. Choose one in AI Models settings."
        );
        return;
      }
      ...
      const providerConfig = PROVIDER_CONFIG[reasoningProvider] || {
        label:
          reasoningProvider.charAt(0).toUpperCase() + reasoningProvider.slice(1),
      };
      const providerLabel = providerConfig.label;

      if (providerConfig.baseStorageKey) {
        const baseUrl = (
          localStorage.getItem(providerConfig.baseStorageKey) || ""
        ).trim();
        if (!baseUrl) {
          setTestResult(
            `${providerLabel} base URL missing. Add it in AI Models settings.`
          );
          return;
        }
      }
      ...
      if (providerConfig.apiKeyStorageKey) {
        ...
        if (!apiKey || apiKey.trim() === "") {
          setTestResult(
            `No ${providerLabel} API key found. Add it in AI Models settings.`
          );
          return;
        }
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          These are the exact prompts currently being sent to your AI models.
          Understanding these helps you see how OpenWhispr thinks!
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="bg-muted border border-border rounded-lg p-4 font-mono text-sm text-foreground">
  ...
  const renderEditPrompts = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Edit these prompts to change how your AI behaves. Use{" "}
          <code className="bg-muted px-1 rounded">{"{{agentName}}"}</code> and{" "}
          <code className="bg-muted px-1 rounded">{"{{text}}"}</code> as
          placeholders.
        </p>
      </div>
  ...
  const renderTestPlayground = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-foreground">
            <TestTube className="size-5 text-green-600 dark:text-green-400" />
            Test Your Prompts
          </h3>
          <p className="text-sm text-muted-foreground mb-6">
            Test your custom prompts with the actual AI model to see real
            results.
          </p>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                  Enable AI text enhancement in the AI Models settings to test
                  prompts.
                </p>
              </div>
            </div>
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-muted-foreground">
                  Try including "{agentName}" in your text to test agent mode
                  prompts
                </p>
                {testText && (
                  <span
            ...
            {testResult && (
              <div>
                ...
                <div className="border border-border rounded-lg p-4 text-sm max-h-60 overflow-y-auto bg-muted text-foreground">
